...
func (service *HTTPRestService) requestIPConfigHandler(w http.ResponseWriter, r *http.Request) {
	...
	logger.Response(service.Name+operationName, reserveResp.PodIpInfo, resp.ReturnCode, ReturnCodeToString(resp.ReturnCode), err)
}
...
func (service *HTTPRestService) releaseIPConfigHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		logger.Errorf("releaseIPConfigHandler decode failed because %v", returnMessage)
		return
	}
	...
	if err = service.releaseIPConfig(podInfo); err != nil {
		statusCode = NotFound
		returnMessage = err.Error()
		logger.Errorf("releaseIPConfigHandler releaseIPConfig failed because %v", returnMessage)
		return
	}
	return
}
...
func (service *HTTPRestService) getIPAddressesHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		returnMessage = err.Error()
		logger.Errorf("getIPAddressesHandler decode failed because %v", returnMessage)
		return
	}
...
func (service *HTTPRestService) setIPConfigAsAvailable(ipconfig cns.IPConfigurationStatus, podInfo cns.KubernetesPodInfo) cns.IPConfigurationStatus {
	...
	logger.Printf("Deleted outdated pod info %s from PodIPIDByOrchestratorContext since IP will be released", podInfo.GetOrchestratorContextKey())
	return service.PodIPConfigState[ipconfig.ID]
}
